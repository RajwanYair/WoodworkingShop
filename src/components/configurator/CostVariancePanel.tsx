...
export function CostVariancePanel() {
  ...
  const entries = useMemo((): MaterialCostEntry[] =>
    ...
    <section className="rounded-lg border border-wood-200 bg-wood-50 dark:border-wood-700 dark:bg-wood-900/30">
      ...
      >
        <span className="flex items-center gap-2 font-semibold text-wood-800 dark:text-wood-100">
          ...
          {hasVariance && (
            <span className={`rounded-full px-1.5 py-0.5 text-xs font-medium ${report.totalVariance > 0 ? 'bg-red-100 text-red-600 dark:bg-red-900/40 dark:text-red-400' : 'bg-green-100 text-green-600 dark:bg-green-900/40 dark:text-green-400'}`}>
              {report.totalVariance > 0 ? '+' : ''}{report.totalVariance.toFixed(0)}
            </span>
          )}
        </span>
      ...
      {open && (
        <div className="border-t border-wood-200 px-4 pb-4 pt-3 dark:border-wood-700">
          {entries.length === 0 ? (
            <p className="text-xs text-wood-400 dark:text-wood-500">{t('costVariance.noData')}</p>
          ) : (
            <>
              <table className="w-full text-sm" aria-label={t('costVariance.tableAriaLabel')}>
                ...
                <tbody>
                  {report.lines.map((line) => (
                    <tr key={line.materialKey} className="border-t border-wood-100 dark:border-wood-800">
                      <td className="py-1 font-mono text-xs text-wood-700 dark:text-wood-300">
                        {line.materialName}
                      </td>
                      <td className="py-1 text-end tabular-nums text-wood-500 dark:text-wood-400 text-xs">
                        {fmt(line.estimatedCost, line.currencyCode)}
                      </td>
                      <td className="py-1 text-end tabular-nums text-xs">
                        {editKey === line.materialKey ? (
                          <input
                            ...
                            className="w-20 rounded border border-wood-300 px-1 text-end text-xs dark:border-wood-600 dark:bg-wood-800"
                            aria-label={t('costVariance.editActualAriaLabel', { key: line.materialName })}
                          />
                        ) : (
                          <button
                            type="button"
                            onClick={() => { setEditKey(line.materialKey); setEditVal(String(line.actualCost)); }}
                      ...
                      <td className={`py-1 text-end tabular-nums text-xs font-medium ${varClass(line.variance)}`}>
                        {line.variance > 0 ? '+' : ''}{fmt(line.variance, line.currencyCode)}
                        <span className="ms-1 text-wood-400">({line.variancePct > 0 ? '+' : ''}{line.variancePct}%)</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr className="border-t-2 border-wood-300 dark:border-wood-600">
                    <td className="py-1 text-xs font-bold text-wood-700 dark:text-wood-200">{t('costVariance.total')}</td>
                    <td className="py-1 text-end text-xs font-bold tabular-nums">{fmt(report.totalEstimated, report.currencyCode)}</td>
                    <td className="py-1 text-end text-xs font-bold tabular-nums">{fmt(report.totalActual, report.currencyCode)}</td>
                    <td className={`py-1 text-end text-xs font-bold tabular-nums ${varClass(report.totalVariance)}`}>
                      {report.totalVariance > 0 ? '+' : ''}{fmt(report.totalVariance, report.currencyCode)}
                    </td>
                  </tr>
                </tfoot>
              </table>
