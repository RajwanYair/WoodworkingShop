...
def build_pdf(lang, cab_key):
    ...
    cover_subtitle = (
        T("מהדורת קטלוג פרימיום לבנייה בעומק מלא")
        if is_he else
        "Premium catalog edition for the full-depth build"
    )
    cover_meta = (
        T(f"אפריל 2026 | תוכנית א' | מידות: {texts['size_str']} מ\"מ")
        if is_he else
        f"April 2026 | Plan A | Dimensions: {texts['size_str']} mm"
    )
    cover_intro = (
        T("תצורת הדגל למטבח: עומק מלא, חזות נקייה, וחומרי גוף עמידים לטווח ארוך.")
        if is_he else
        "Flagship kitchen-pantry layout with full depth, clean front elevations, and the most durable carcass material in the collection."
    )
    story.extend(make_catalog_cover(
        ...
        cover_intro,
        [
            (T("עומק") if is_he else "Depth", f"{cab['ext'][2]} mm"),
            (T("גיליונות 17 מ\"מ") if is_he else "17 mm sheets", "5"),
            (T("חומר") if is_he else "Material", T("סנדוויץ' פרימיום") if is_he else "Premium sandwich plywood"),
        ],
        sty,
        is_he=is_he,
    ...
    product_specs = [
        ...
        (T("מפתח דלת") if is_he else "Door program", f"2 x {cab['door_h']} x {cab['door_w']}"),
        (T("רוחב פנימי") if is_he else "Internal width", str(cab['int_w'])),
        (T("גובה פנימי") if is_he else "Internal height", str(cab['int_h'])),
    ]
    story.append(make_product_card(
        T("כרטיס מוצר") if is_he else "Product Card",
        T("הצורה הקלאסית, עומק מלא, ושפת נגרות נקייה") if is_he else "The classic full-depth version with the most generous storage volume.",
        product_specs,
        T("מומלץ כאשר המראה, העומק והעמידות מקבלים עדיפות על פני חיסכון בחומר.") if is_he else "Best when visual quality, full reach depth, and premium sheet goods matter more than sheet-count savings.",
        sty,
        is_he=is_he,
    ))
    story.append(Spacer(1, 8))
    story.append(make_feature_callout(
        T("קריאות עיצוב") if is_he else "Design Callouts",
        [
            T("עומק מלא של 600 מ\"מ לפריטי מזווה גדולים ולגישה נוחה יותר.") if is_he else "Full 600 mm depth for bulk pantry storage and the strongest visual presence.",
            T("קנטים מסומנים בכל הקצוות הגלויים למראה מטבח נקי ומוגמר.") if is_he else "Visible front edges are fully finished for a cleaner furniture-grade presentation.",
            T("כל הגוף, הדלתות והגב מיוצרים מסנדוויץ' להתאמה מלאה בשפה החומרית.") if is_he else "All carcass, door, and back panels stay in sandwich plywood for a fully consistent material spec.",
        ],
        sty,
        is_he=is_he,
    ))
